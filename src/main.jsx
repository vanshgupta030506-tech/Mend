...
function Describe({ go, patch, state }) { return <div><Card><b>{state.issue || 'Unusual noise'}</b><p>LG Washing Machine · Spin cycle</p></Card><textarea defaultValue={state.description || 'Makes a loud grinding noise while spinning, especially with heavier clothes.'} onChange={e=>patch({description:e.target.value})} placeholder="Describe what you noticed" /><div className="tabs"><button>Always</button><button>Sometimes</button><button>Spin</button></div><Primary onClick={()=>go('media')}>Continue</Primary></div>; }
